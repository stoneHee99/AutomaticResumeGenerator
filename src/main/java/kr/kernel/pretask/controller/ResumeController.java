...
public class ResumeController {
    ...
    private void createResumeSheet(PersonInfo personInfo, List<Education> educationList, List<Career> careerList) {
        ...
        createPersonInfoHeader(sheet);

        Row personInfoDataRow = sheet.createRow(1);
        insertPhotoToResume(personInfo, sheet, personInfoDataRow);
        createPersonInfoData(personInfoDataRow, personInfo);

        int educationStartRow = 3;
        createEducationHeader(sheet, educationStartRow);

        educationStartRow = createEducationData(educationList, sheet, educationStartRow);

        int careerStartRow = educationStartRow + 1;
        createCareerHeader(sheet, careerStartRow);

        createCareerData(careerList, sheet, careerStartRow);
    }

    private void createPersonInfoHeader(Sheet sheet) {
        Row personInfoHeaderRow = sheet.createRow(0);
    ...
    private void insertPhotoToResume(PersonInfo personInfo, Sheet sheet, Row row) {
    ...
    private void createEducationHeader(Sheet sheet, int rowNum) {
        Row educationHeaderRow = sheet.createRow(rowNum - 1);
    ...
    private int createEducationData(List<Education> educationList, Sheet sheet, int rowNum) {
        for (Education education : educationList) {
            Row educationDataRow = sheet.createRow(rowNum++);
    ...
    private void createCareerHeader(Sheet sheet, int rowNum) {
        Row careerHeaderRow = sheet.createRow(rowNum - 1);
    ...
    private void createCareerData(List<Career> careerList, Sheet sheet, int rowNum) {
        for (Career career : careerList) {
            Row careerDataRow = sheet.createRow(rowNum++);
